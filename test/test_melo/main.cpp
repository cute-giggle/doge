...
int main() {
    ...
    const auto model_path = current_path + "/../../model/melo/melo-zh-en.xml";
    ov::CompiledModel compiled_model = core.compile_model(model_path, "AUTO");
    ov::InferRequest infer_request = compiled_model.create_infer_request();

    std::string text =
        "在这个示例中，我们使用PortAudio库来实现实时播放音频数据。paCallback函数是PortAudio的回调函数，用于将音频数据写入输出缓"
        "冲区。main函数中初始化PortAudio，打开默认音频流，并开始播放音频数据。播放完成后，关闭音频流并终止PortAudio。";
    auto preprocesser = doge::model::MeloPreprocessor();
    auto sentences = preprocesser.split(text, 16);
    std::vector<float> result;

    for (auto i = 0U; i < sentences.size(); ++i) {
        auto model_input = preprocesser.preprocess(sentences[i], 1.25);
        ...
        const auto start = doge::utils::current_ts_ms();
        infer_request.start_async();
        infer_request.wait();
        spdlog::debug("Infer request done, time cost: {} ms", doge::utils::current_ts_ms() - start);
    ...
    doge::utils::save_wav(save_path, result.data(), result.size(), 44100, 1);
