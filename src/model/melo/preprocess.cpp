...
std::vector<std::string> MeloPreprocessor::split(const std::string &sentence, size_t min_len) const {
    ...
    const auto norm_sentence = doge::SentenceNormalizer::normalize(sentence);
    static const std::unordered_set<char> sep_set = doge::SentenceNormalizer::get_supported_punctuations();
...
doge::model::InputData MeloPreprocessor::preprocess(const std::string &text, float speed) const {
    std::string temp = " " + text + " ";
    const auto pronounces = doge::language::LanguageHelper::instance().pronounce(temp);
    ...
    for (auto [phone, tone, lang] : pronounces) {
        ...
        tones.push_back(lang == doge::language::Language::EN ? tone + EN_TONE_START : tone + ZH_TONE_START);
    }
    ...
    doge::model::InputData result;
    const size_t seq_len = phones.size();

    result["phones"] = doge::model::Data{std::move(phones_buffer), {1, seq_len}};
    result["tones"] = doge::model::Data{std::move(tones_buffer), {1, seq_len}};
    result["langs"] = doge::model::Data{std::move(langs_buffer), {1, seq_len}};
    ...
    *reinterpret_cast<int64_t *>(phones_len.data()) = seq_len;
    ...
    result["speed"] = doge::model::Data{std::move(speed_buffer), {1}};
    result["speakers"] = doge::model::Data{std::move(speakers), {1}};
    result["phones_len"] = doge::model::Data{std::move(phones_len), {1}};

    std::vector<uint8_t> bert1(1 * 1024 * seq_len * sizeof(float), 0);
    std::vector<uint8_t> bert2(1 * 768 * seq_len * sizeof(float), 0);

    result["bert1"] = doge::model::Data{std::move(bert1), {1, 1024, seq_len}};
    result["bert2"] = doge::model::Data{std::move(bert2), {1, 768, seq_len}};
