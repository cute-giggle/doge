...
void LanguageHelper::initialize_jieba() {
    ...
    const auto start = doge::utils::current_ts_us();
    LanguageHelper::jieba_ = std::make_unique<cppjieba::Jieba>(pts[0], pts[1], pts[2], pts[3], pts[4]);
    spdlog::debug("Initialize cppjieba done, time cost: {} us", doge::utils::current_ts_us() - start);
}
...
void LanguageHelper::get_unicode_pinyin_list() {
    ...
    spdlog::debug("Reading unicode pinyin file done, time cost: {} us", doge::utils::current_ts_us() - start);
}
...
void LanguageHelper::get_chinese_pinyin_phone_mapper() {
    ...
    spdlog::debug("Reading Chinese pinyin phone mapper file done, time cost: {} us", doge::utils::current_ts_us() - start);
}
...
void LanguageHelper::get_chinese_words_pinyin_mapper() {
    ...
    spdlog::debug("Reading chinese words pinyin mapper file done, time cost: {} us", doge::utils::current_ts_us() - start);
}
...
void LanguageHelper::get_tagging_words_pinyin_mapper() {
    ...
    spdlog::debug("Reading tagging words pinyin mapper file done, time cost: {} us", doge::utils::current_ts_us() - start);
}
...
void LanguageHelper::get_english_words_pinyin_mapper() {
    ...
    spdlog::debug("Reading english words pinyin mapper file done, time cost: {} us", doge::utils::current_ts_us() - start);
}
...
std::vector<Pronounce> LanguageHelper::pronounce(const std::string &sentence) const {
    ...
    for (const auto &[phone, tone, lang] : result) {
        pronounce_logstr += spdlog::fmt_lib::format("({}|{}|{})", phone, tone, doge::utils::to_underlying(lang));
    }
...
std::vector<std::string> LanguageHelper::chinese_pinyin(const Token &word) const {
    ...
    if (citer != chinese_words_pinyin_mapper_.end()) {
        return doge::utils::string_split_any(citer->second, " ");
    }
    ...
    if (titer != tagging_words_pinyin_mapper_.end()) {
        return doge::utils::string_split_any(titer->second, " ");
    }
...
std::vector<std::string> LanguageHelper::english_pinyin(const Token &token) const {
    const auto word = doge::utils::string_toupper(token.first);
    const auto iter = english_words_pinyin_mapper_.find(word);
    if (iter == english_words_pinyin_mapper_.end()) {
        return {};
    }
    return doge::utils::string_split_all(iter->second, " - ");
}
...
std::vector<Pronounce> LanguageHelper::english_pronounce(const std::string &pinyin) const {
    std::vector<Pronounce> result;
    auto phone_list = doge::utils::string_split_any(pinyin, " ");
    for (auto &phone : phone_list) {
        ...
        phone = doge::utils::string_tolower(phone);
